...
public class TeleportUtils {
   public static void genericTeleport(boolean bossBar, double standStillTime, ServerPlayerEntity who, Runnable onCounterDone) {
      ...
      if (bossBar) {
         standStillBar = server.getBossBarManager().add(new Identifier("standstill-" + who.getUuidAsString()), MutableText.of(LiteralTextContent.EMPTY));
         ...
         public void run() {
            ...
            if (bossBar) {
               ...
               whoFinal[0].sendMessage(MutableText.of(new LiteralTextContent("Stand still for ")).formatted(Formatting.LIGHT_PURPLE)
                     .append(MutableText.of(new LiteralTextContent(Integer.toString((int) Math.floor(counter[0] + 1)))).formatted(Formatting.GOLD))
