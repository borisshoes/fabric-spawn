package net.borisshoes.fabricspawn.utils;
...
public class ConfigUtils {
   ...
   public LiteralArgumentBuilder<ServerCommandSource> generateCommand(String commandName){
      LiteralArgumentBuilder<ServerCommandSource> out =
            literal(commandName).requires(source -> source.hasPermissionLevel(4))
                  .executes(ctx -> {
                     values.stream().filter(v -> v.command != null).forEach(value ->
                           ctx.getSource().sendFeedback(MutableText.of(new TranslatableTextContent(value.command.getterText, value.value)), false));
                     return 1;
                  });
      values.stream().filter(v -> v.command != null).forEach(value ->
            out.then(literal(value.name)
                  ...
                  .then(argument(value.name, value.getArgumentType()).suggests(value.suggestions)
                        .executes(ctx -> {
                           ...
                           ((CommandContext<ServerCommandSource>) ctx).getSource().sendFeedback(MutableText.of(new TranslatableTextContent(value.command.setterText, value.value)), true);
                           this.save();
                           return 1;
                        }))));
      return out;
